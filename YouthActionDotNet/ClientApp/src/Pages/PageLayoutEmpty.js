...
export default class DatapageLayoutEmpty extends React.Component {
  ...
  componentDidMount = async () => {
    ...
    const perms = await this.props.permissions.find(
      (p) => p.Module === this.props.settings.title
    );
  ...
  populateActions = async (perms, components) => {
    ...
    if (perms?.Create && this.props.has?.Create) {
      tableHeaderActions.push({
        label: "Add " + this.props.settings.title,
        onClick: () => {
          this.setExpansionContent("add");
        },
      });
    }
    if (perms?.Delete && this.props.has?.Delete) {
      tableHeaderActions.push({
        label: "Delete " + this.props.settings.title,
        onClick: () => {
          this.setExpansionContent("del");
        },
      });
    }
    ...
    components.forEach((component) => {
      tableHeaderActions.push({
        ...
        onClick: () => {
          this.setExpansionContent(component.key);
        },
      });
    });
    ...
    return this.state.perms?.Read ? (
      <div className="d-flex flex-column container-fluid listPageContainer h-100">
        ...
        <div className="col-12 d-flex flex-column h-100">
          <TableHeader
            actions={this.state.tableHeaderActions}
      ...
      <AccessDeniedPanel></AccessDeniedPanel>
    );
  }
}
    ...
    return (
      <div className="tableHeader">
        <div
          className={
            "tableHeaderActions " +
            (this.props.component === "" ? "borderRadius" : "topBorderRadius")
          }
        >
            ...
            <IconButton
              ...
              icon={
                <i
                  ...
                  onClick={this.props.requestRefresh}
                ></i>
              }
            ></IconButton>
            {this.props.showBottomMenu ? (
  ...
  render() {
    return (
      <div className="p-4 headerExpansionPane">
        <div
          className={
            "panelCloseBtn d-flex justify-content-between align-items-center"
          }
        >
  ...
  render() {
    return (
      <div className="container-fluid addEntry">
        <form
          className={"addEntry-inputFields"}
          onSubmit={this.handleCourseCreation}
        >
  ...
  render() {
    return (
      <div className="container-fluid deleteEntry">
        <form
          className={"deleteEntry-inputFields"}
          onSubmit={this.handleCourseDeletion}
        >
          {Object.keys(this.props.fieldSettings).map((key, index) => {
            return this.props.fieldSettings[key].primaryKey ? (
              <StdInput
        ...
        <div className="generate-actions">
          ...
          {this.state.spreadsheetReady ? (
            <CSVLink
              ...
              filename={this.props.settings.title + ".csv"}
            >
              Download
            </CSVLink>
          ) : (
...
class TableFooter extends React.Component {
  render() {
    return this.props.showBottomMenu ? (
      <ActionsButton
